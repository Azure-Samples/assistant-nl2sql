...
class SampleDatabaseCreator:
    ...
    def create_dataset_and_tables(self, dataset_name):
        ...
        table_sales_transaction = bigquery.Table(table_id_sales_transaction, schema=schema_sales_transaction)
    ...
    def populate_sample_data(self, dataset_name):
        ...
        # Insert Products
        products = [
            (1, 'Apple iPhone 13', 'Latest model of iPhone with A15 Bionic chip', 999.99, 'Electronics', True),
            (2, 'Nike Air Force 1', 'Classic Nike sneakers in white', 90.00, 'Footwear', True),
            (3, 'The Alchemist', 'A novel by Paulo Coelho', 10.99, 'Books', True),
            (4, 'Apple iPhone 12', 'Previous model of iPhone with A14 Bionic chip', 799.99, 'Electronics', True),
            (5, 'Adidas Ultraboost', 'Running shoes with Boost cushioning', 180.00, 'Footwear', True),
            (6, 'To Kill a Mockingbird', 'A novel by Harper Lee', 12.99, 'Books', True),
            (7, 'Apple iPhone 11', 'Older model of iPhone with A13 Bionic chip', 699.99, 'Electronics', False),
            (8, 'Vans Old Skool', 'Classic Vans sneakers in black and white', 60.00, 'Footwear', True),
            (9, '1984', 'A novel by George Orwell', 9.99, 'Books', True),
            (10, 'Samsung Galaxy S21', 'Latest model of Samsung Galaxy with Exynos 2100 chip', 899.99, 'Electronics', True),
        ]
        ...
        with open(products_csv_path, 'w', newline='') as csvfile:
            ...
            writer.writerow(['product_id', 'product_name', 'product_description', 'product_price', 'product_category', 'in_stock'])
        ...
        with open(products_csv_path, 'rb') as source_file:
            job = self.client.load_table_from_file(source_file, table_id_products, job_config=job_config)
        ...
        with open(sellers_csv_path, 'w', newline='') as csvfile:
            writer = csv.writer(csvfile)
            writer.writerow(['seller_id', 'seller_name', 'seller_email', 'seller_contact_number', 'seller_address'])
            writer.writerows(sellers)

        table_id_sellers = f"{dataset_id}.sellers"
        with open(sellers_csv_path, 'rb') as source_file:
            job = self.client.load_table_from_file(source_file, table_id_sellers, job_config=job_config)
        job.result()  # Wait for the job to complete
        ...
        for i in range(100):
            ...
            sales_transactions.append((transaction_id, product_id, seller_id, quantity, transaction_date))

        sales_transactions_csv_path = os.path.join(os.path.dirname(__file__), 'sales_transactions.csv')
        with open(sales_transactions_csv_path, 'w', newline='') as csvfile:
            writer = csv.writer(csvfile)
            writer.writerow(['transaction_id', 'product_id', 'seller_id', 'quantity', 'transaction_date'])
            writer.writerows(sales_transactions)

        table_id_sales_transaction = f"{dataset_id}.sales_transaction"
        with open(sales_transactions_csv_path, 'rb') as source_file:
            job = self.client.load_table_from_file(source_file, table_id_sales_transaction, job_config=job_config)
        job.result()  # Wait for the job to complete
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Create and populate a sample BigQuery database.")
    parser.add_argument("dataset_name", type=str, help="The name of the dataset to create.")
